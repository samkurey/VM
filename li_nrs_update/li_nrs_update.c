						   ...
						   )
{
	...
	userlog("li_nrs_update_p : email - %s", req.misc.email); //added by samsuri on 20 oct 2022
	...
	userlog("li_nrs_update_p : trans_type : %d",req.misc.trans_type);
	...
	else
	{
		...
		#ifdef DEBUG
			userlog("After sending the blank city_code to host req->new_address->city_code :%d:",req.new_address.city_code);
		#endif
	}
	...
	if((req.misc.email[0] != '\0') && (req.misc.email[0] != ' '))
	{
	ret = add_one_fc_field(FID_EMAIL, &temp_ptr,  req.misc.email,	
					TYPE_CHAR_ARR, error_code);
	}
	userlog(" req.misc.email: %s",req.misc.email);
	...
	/*trans_type*/
	temp_char_arr[0] = '\0';
	userlog(" req.misc.trans_type: %d",req.misc.trans_type);
	printf("Trans type [%d]", req.misc.trans_type);
	if(req.misc.trans_type != 0)
	{
		itoa(req.misc.trans_type, temp_char_arr, 10);
		ret = add_one_fc_field(FID_TRANS_TYPE, &temp_ptr, temp_char_arr,
					TYPE_CHAR_ARR, error_code);	
	}
...
long get_buffer_size1(LI_NRS_UPDATE_REQ_T *req)
{
	...
	len += ID_SIZE + ID_TYPE_SIZE + TXNCODE_SIZE + KPP_SIZE + DATE_SIZE
		   ...
		   + CHAR_SIZE + DATE_SIZE + CONTACTNO_SIZE + 101; //added by samsuri on 20 oct 2022
