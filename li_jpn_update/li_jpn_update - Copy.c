...
#include <stdlib.h>
#include <stdio.h>
#include "D:\GMPCServices\COMMON_FILES\userlog.h"
...
// My Tentera end
	...
	strcpy(lkptno,req.misc.applicant_kpt);

	/* Copy the images */	
	ret = copy_images(&req, &temp_ptr, error_code,lkptno);
	if(GMPC_GOOD != ret)
	{
		userlog("Inside the copy_images, failed, KPTNO = %s", req.misc.applicant_kpt);		
		free(put_var_data.data);
		return ret;
	}
...
long judge_buffer_size(LI_JPN_UPDATE_REQ_T req)
{
	...
	*/
	...
	len = len + FEE_SIZE;      // added on 12/09/2008 for MYPR extrafee
...
int copy_images(LI_JPN_UPDATE_REQ_T *req, char **in_temp_ptr, int *error_code, char lkptno[12])
{
...
*/
	...
	if(req->low_res_portrait.portrait_size > 0)
	{
	 ...
	 if (ret != GMPC_GOOD)
	 {
		userlog("Unable to compress the low res photo, kptno = %s",lkptno);
		return GMPC_BAD;
	 }
			...
			if( dwBufSize > dwMaxSize )	{
				...
				*actual_len = dwBufSize;
				...
				if( loadjpgfrombuffer(pBuffer, desImage) != NO_ERROR)	{
					...
					if(nQuality == 100)
					{
						userlog("In the Quality = %d retuning false\n", nQuality);
						return FALSE;
					}
					else
					{
